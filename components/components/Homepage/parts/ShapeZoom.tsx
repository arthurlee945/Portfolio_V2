...
import { FC, SyntheticEvent, useEffect, useState, Suspense, useRef, useCallback } from "react";
...
interface SZProps {}
const ShapeZoomContainer = styled.div<{ $sceneState: boolean; $playing: boolean; $speed: number }>`
  position: relative;
  display: grid;
  place-items: center;
  ...
  ${({ $sceneState, $playing, $speed }) =>
    $sceneState &&
    $playing &&
    css`
      &:before {
        animation: ground-move ${$speed === 5 ? "1500ms" : $speed === 3 ? "2500ms" : "3500ms"} linear infinite;
...
interface RunnerModelProps {
  playing?: boolean;
  speed?: number;
}
const RunnerModel: FC<RunnerModelProps> = ({ playing, speed }) => {
  ...
  useEffect(() => {
    if (playing) {
      ...
      actions[names[0]]?.setEffectiveTimeScale(speed || 2.5);
...
const ShapeZoom: FC<SZProps> = ({}) => {
  ...
  const handleSpeedButtons = (buttonType: "fast" | "slow") => {
    if ((buttonType === "fast" && sceneState.speed === 5) || (buttonType === "slow" && sceneState.speed === 2) || !sceneState.playing)
      return;
    setSceneState((curr) => ({
      ...curr,
      speed: buttonType === "fast" ? (curr.speed === 2 ? 3 : 5) : curr.speed === 5 ? 3 : 2,
    }));
  };

  return (
    <ShapeZoomContainer ref={szRef} $sceneState={sceneState.started} $playing={sceneState.playing} $speed={sceneState.speed}>
      <AnimatePresence mode="wait">
        {!sceneState.started ? (
          ...
          >
            <button className="toggle-button" onClick={handleAnimationToggle}>
              <span></span>
              <span></span>
              <span></span>
              <span></span>
            </button>
            <button className="speed-button rewind" onClick={handleSpeedButtons.bind(null, "slow")}>
              <span></span>
              <span></span>
              <span></span>
              <span></span>
            </button>
            <button className="speed-button forward" onClick={handleSpeedButtons.bind(null, "fast")}>
              <span></span>
              <span></span>
              <span></span>
              <span></span>
      ...
      <Canvas className="model-canvas" gl={{ alpha: true }}>
        ...
        <Suspense fallback={null}>
          <RunnerModel playing={sceneState.started && sceneState.playing} speed={sceneState.speed} />
        </Suspense>
      </Canvas>
    </ShapeZoomContainer>
  );
